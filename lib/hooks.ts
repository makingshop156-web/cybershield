...
function computeBadges(completed: string[], streak: number): string[] {
  ...
  for (const mod of modules) {
    ...
    if (badgeId && mod.lessons.every((l) => completed.includes(l.id))) {
      badges.push(badgeId);
    }
  }

  const allLessonIds = modules.flatMap((m) => m.lessons.map((l) => l.id));
  if (allLessonIds.length > 0 && allLessonIds.every((id) => completed.includes(id))) {
    badges.push("all_modules");
  }
...
export function useProgress(): {
  ...
  const completeLesson = useCallback((lessonId: string) => {
    setProgress((prev) => {
      ...
      } else if (prev.lastStudyDate === getYesterdayISO()) {
      ...
      return {
        ...
        streak: newStreak,
        lastStudyDate: today,
        badges: computeBadges(updated, newStreak),
      };
    });
  }, []);
...
export function useSpeedrun(): [boolean, () => void] {
  ...
  useEffect(() => {
    setSpeedrun(getStorageItem(SPEEDRUN_KEY, false));
  }, []);

  const toggleSpeedrun = useCallback(() => {
    setSpeedrun((prev) => {
      const next = !prev;
      setStorageItem(SPEEDRUN_KEY, next);
      return next;
    });
  }, []);
